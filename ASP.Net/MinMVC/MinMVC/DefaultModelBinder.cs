...
namespace MinMVC
{
    public class DefaultModelBinder : IModelBinder
    {
        ...
        private bool GetValueTypeInstance(ControllerContext controllerContent, string modelName, Type modelType, out object value)
        {
            ...
            key = controllerContent.RequestContext.RouteData.Values
                .Where(item => string.Compare(item.Key, modelName, true) == 0)
                .Select(item => item.Key).FirstOrDefault();
            if (key != null)
            {
                value = Convert.ChangeType(controllerContent.RequestContext.RouteData.Values[key], modelType);
                return true;
            }
            key = controllerContent.RequestContext.RouteData.DataTokens
                .Where(item => string.Compare(item.Key, modelName, true) == 0)
                .Select(item => item.Key).FirstOrDefault();
            if (key != null)
            {
                value = Convert.ChangeType(controllerContent.RequestContext.RouteData.DataTokens[key], modelType);
                return true;
            }
            value=null;
            return false;
